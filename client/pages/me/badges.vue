  ...
  <div class="p-4 md:p-6">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
      <div v-for="b in catalog" :key="b.key" class="rounded-xl p-4 border dark:border-white/10">
        ...
        <div v-if="!earnedSet.has(b.key)" class="mt-2 text-xs opacity-80">
          {{ Math.min(me.total, b.count) }}/{{ b.count }}
        </div>
      </div>
    </div>
  </div>
...
export default {
  async fetch({ store }) {
    await Promise.all([
      store.dispatch('achievements/fetchCatalog'),
      store.dispatch('achievements/fetchMe')
    ])
  },
  computed: {
    me() {
